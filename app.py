...
app.config['UPLOAD_FOLDER'] = str(
  Path.home())  # Set upload folder to the user's home directory


def allowed_file(filename):
  return '.' in filename and filename.rsplit('.',
                                             1)[1].lower() in ['xlsx', 'xls']
...
def myform():
  if request.method == 'POST':
    ...
    if file and allowed_file(file.filename):
      ...
      # Create a new Excel file for writing the summary sheet
      summary_filename = str(uuid.uuid4()) + '_summary.xlsx'
      summary_output_path = os.path.join(app.config['UPLOAD_FOLDER'],
                                         summary_filename)

      # Write the original DataFrame to the first sheet of the summary Excel file
      with pd.ExcelWriter(summary_output_path, engine='openpyxl') as writer:
        df.to_excel(writer, sheet_name='Data', index=False)

        # Create a summary DataFrame with the required summary information
